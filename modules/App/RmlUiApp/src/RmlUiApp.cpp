  ...
  std::visit(
    Overload{
      [this](const os::MouseMoveEvent &evt_) {
        getUiContext().ProcessMouseMove(evt_.position.x, evt_.position.y, 0);
      },
      [this](const os::MouseButtonEvent &evt_) {
        ...
        case Pressed:
          getUiContext().ProcessMouseButtonDown(btn, 0);
          break;

        case Released:
          getUiContext().ProcessMouseButtonUp(btn, 0);
          break;
        }
      },
      [this](const os::MouseWheelEvent &evt_) {
        ...
        *(evt_.wheel == os::MouseWheel::Horizontal ? &val.x : &val.y) +=
          -evt_.step;
        getUiContext().ProcessMouseWheel(val, 0);
      },
      [this](const os::KeyboardEvent &evt_) {
        ...
        case Down:
          getUiContext().ProcessKeyDown(ki, 0);
          break;

        case Up:
          getUiContext().ProcessKeyUp(ki, 0);
          break;
        }
      },
      [this](const os::InputCharacterEvent &evt_) {
        getUiContext().ProcessTextInput(evt_.c);
      },
    },
    evt);
}
