...
bool CreateNodeCommand::execute(const Context &ctx) {
  ...
  ctx.logger.info("[{}][node_id={}] Created (factory_id={})", __FUNCTION__,
                  *m_id, m_factoryId);
  return true;
}
...
bool CloneNodeCommand::execute(const Context &ctx) {
  ...
  if (const auto sourceVd = g->findVertex(m_sourceId); sourceVd) {
    ...
    m_id = vertex.id;
    ctx.logger.info("[{}][node_id={}] Created (source_node_id={})",
                    __FUNCTION__, *m_id, m_sourceId);
    ...
    ctx.logger.error("[{}][source_node_id={}] Not Found!", __FUNCTION__,
                     m_sourceId);
    return false;
  }
}
