...
std::string TextEditor::GetText(const Coordinates &aStart,
  ...
  while (istart < iend || lstart < lend) {
    ...
    if (istart < (int)line.size()) {
      ...
      istart = 0;
      ++lstart;
      // https://github.com/BalazsJako/ImGuiColorTextEdit/issues/117
  ...
  if (!mLines.empty()) {
    ...
    while (lineNo <= lineMax) {
      ...
      if (mState.mCursorPosition.mLine == lineNo) {
        ...
        if (!HasSelection()) {
          auto end =
            ImVec2(start.x + contentSize.x + scrollX, start.y + mCharAdvance.y);
      ...
      for (int i = 0; i < line.size();) {
        ...
        if ((color != prevColor || glyph.mChar == '\t' || glyph.mChar == ' ') &&
          ...
          auto textSize = ImGui::GetFont()->CalcTextSizeA(
            ImGui::GetFontSize(), FLT_MAX, -1.0f, mLineBuffer.c_str(), nullptr,
            nullptr);
...
void TextEditor::MoveEnd(bool aSelect) {
  auto oldPos = mState.mCursorPosition;
  SetCursorPosition(
    Coordinates(mState.mCursorPosition.mLine, GetLineMaxColumn(oldPos.mLine)));
...
const TextEditor::LanguageDefinition &TextEditor::LanguageDefinition::GLSL() {
  ...
  if (!inited) {
    ...
    for (auto &k : identifiers) {
      ...
      id.mDeclaration = "Built-in function";
      if (strcmp(k, "sin") == 0) {
        id.mDeclaration = "genType sin(genType angle)\nangle\n\tSpecify the "
                          "quantity, in radians, of which to return the sine.";
      }
      langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
    }
...
TextEditor::LanguageDefinition::AngelScript() {
  ...
  if (!inited) {
    static const char *const keywords[] = {
      "and",       "abstract", "auto",    "bool",     "break",     "case",
      "cast",      "class",    "const",   "continue", "default",   "do",
      "double",    "else",     "enum",    "false",    "final",     "float",
      "for",       "from",     "funcdef", "function", "get",       "if",
      "import",    "in",       "inout",   "int",      "interface", "int8",
      "int16",     "int32",    "int64",   "is",       "mixin",     "namespace",
      "not",       "null",     "or",      "out",      "override",  "private",
      "protected", "return",   "set",     "shared",   "super",     "switch",
      "this ",     "true",     "typedef", "uint",     "uint8",     "uint16",
