...
template <typename T> struct ImObjectPool {
  ...
  ImObjectPool(ImNodesEditorContext *ctx)
      : Owner(ctx), Pool(), InUse(), FreeList(), IdMap() {}
};
...
struct ImNodesEditorContext {
  ...
  ImNodesEditorContext()
      : Nodes(this), Pins(this), Links(this), Panning(0.f, 0.f),
        SelectedNodeIndices(), SelectedLinkIndices(), SelectedNodeOffsets(),
...
inline int ObjectPoolFindOrCreateIndex(ImObjectPool<ImNodeData> &nodes,
  ...
  if (node_idx == -1) {
    ...
    nodes.Owner->NodeDepthOrder.push_back(node_idx);
  }
